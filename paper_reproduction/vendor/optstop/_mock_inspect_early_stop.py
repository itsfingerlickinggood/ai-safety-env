"""Minimal in-package stub of inspect_ai's ``EarlyStopping`` protocol.

This mirrors the ``EarlyStopping`` / ``EarlyStop`` interface from inspect_ai
(MIT-licensed, UK AI Security Institute) so that ``optstop`` still imports when
inspect_ai is not installed - e.g. for standalone post-hoc / live analysis that
does not use the inspect_ai bridge. ``early_stopping.py`` falls back to these
stubs only when ``import inspect_ai`` fails; with inspect_ai installed, the real
classes are used and this module is never imported.
"""
